import { Link, useLocation } from "react-router-dom";
import styles from "./CurrentLocation.module.scss";
import homeIcon from "../../../public/img/House.svg";
import { useTranslation } from "react-i18next";
import { BreadcrumbsT } from "../../types/Breadcrumbs";

type BreadcrumbsKeys = keyof BreadcrumbsT;
export const CurrentLocation = () => {
  const { pathname } = useLocation();
  const { t } = useTranslation();

  const paths = pathname.slice(1).split("/");

  const category = paths[0] as BreadcrumbsKeys;
  const nameFromPath = paths[1] ? paths[1].replaceAll("-", " ") : "";

  const BREADCRUMBS: BreadcrumbsT = {
    phones: t("location.phones"),
    tablets: t("location.tablets"),
    accessories: t("location.accessories"),
    favourites: t("location.favourites"),
    cart: t("location.cart"),
  };

  return (
    <div className={styles.currentLocation}>
      <Link to="/">
        <img src={homeIcon} alt="home-icon" />
      </Link>
      <svg
        width="16"
        height="16"
        viewBox="0 0 16 16"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          fillRule="evenodd"
          clipRule="evenodd"
          d="M5.52876 3.52851C5.78911 3.26816 6.21122 3.26816 6.47157 3.52851L10.4716 7.52851C10.7319 7.78886 10.7319 8.21097 10.4716 8.47132L6.47157 12.4713C6.21122 12.7317 5.78911 12.7317 5.52876 12.4713C5.26841 12.211 5.26841 11.7889 5.52876 11.5285L9.05735 7.99992L5.52876 4.47132C5.26841 4.21097 5.26841 3.78886 5.52876 3.52851Z"
          fill="#4A4D58"
        />
      </svg>
      <Link
        to={`/${category}`}
        className={nameFromPath ? "" : styles.currentLocationName}
      >
        {BREADCRUMBS[category]}
      </Link>
      {nameFromPath && (
        <>
          <svg
            width="16"
            height="16"
            viewBox="0 0 16 16"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M5.52876 3.52851C5.78911 3.26816 6.21122 3.26816 6.47157 3.52851L10.4716 7.52851C10.7319 7.78886 10.7319 8.21097 10.4716 8.47132L6.47157 12.4713C6.21122 12.7317 5.78911 12.7317 5.52876 12.4713C5.26841 12.211 5.26841 11.7889 5.52876 11.5285L9.05735 7.99992L5.52876 4.47132C5.26841 4.21097 5.26841 3.78886 5.52876 3.52851Z"
              fill="#4A4D58"
            />
          </svg>
          <p className={styles.currentLocationName}>{nameFromPath}</p>
        </>
      )}
    </div>
  );
};
